...
namespace Nom.Bytecode
{
    public class ClassRep : InterfaceRep, IClassSpec
    {
        ...
        public IEnumerable<IInstanceSpec> Instances => instances;

        //TODO: add Instance to the byte code
        public void AddInstance(InstanceDefRep idp)
        {
            instances.Add(idp);
        }
        ...
        protected override void EmitBody(Stream ws)
        {
            ...
            UInt64 instanceCount = (ulong)Instances.LongCount();
            ws.WriteValue(instanceCount);
            foreach (InstanceDefRep cdr in instances)
            {
                cdr.WriteByteCode(ws);
            }
        ...
        new public static ClassRep Read(Stream ws, IReadConstantSource rcs)
        {
            ...
            UInt64 instanceCount = ws.ReadULong();
            for (UInt64 i = 0; i < instanceCount; i++)
            {
                cls.AddInstance(InstanceDefRep.Read(cls, ws, rcs));
            }
