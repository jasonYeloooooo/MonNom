...
namespace Nom.Bytecode
{
    public class CodeGenerator
    {
        ...
        public ClassRep ConvertClass(Nom.TypeChecker.IClass cls, AssemblyUnit au /*BytecodeUnit bcu*/)
        {
            ...
            // need to add a IInstanceDef
            foreach (TypeChecker.IInstanceDef id in cls.Instances) {
                List<IInstruction> i = new List<IInstruction>();
                foreach (TypeChecker.IInstruction instruction in id.Instructions)
                {
                    i.Add(instruction.Visit(InstructionConverter.Instance, bcu));
                }
                var n = id.Name;

                InstanceDefRep idr = new InstanceDefRep( bcu.GetTypeListConstant(id.Parameters.Entries.Select(ps => ps.Type)), id.Visibility, i, id.RegisterCount, bcu.GetStringConstant(id.Name),id.Parameters);
                cr.AddInstance(idr);
            }
        ...
        private class InstructionConverter : IInstructionVisitor<BytecodeUnit, IInstruction>
        {
            ...
            public Func<CallInstanceCheckedInstruction, BytecodeUnit, IInstruction> VisitCallInstanceCheckedInstruction => (instr, bcu) =>
          new CallCheckedInstanceInstruction(bcu.GetSuperClassConstant(instr.Instance.Element.Container.MakeClassRef(instr.Instance.Substitutions.Transform(t => t.AsType))), bcu.GetTypeListConstant(instr.ActualParameters), instr.Arguments.Select(reg => reg.Index), instr.Register.Index);
