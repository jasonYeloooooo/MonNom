...
namespace Nom.TypeChecker
{
    class CodeTransformer : Parser.IStmtVisitor<ICodeTransformEnvironment, IStmtTransformResult>, Parser.IExprVisitor<ICodeTransformEnvironment, IExprTransformResult>
    {
        ...
        public Func<DefaultExprStmt, ICodeTransformEnvironment, IStmtTransformResult> VisitDefaultExprStmt => (stmt, env) =>
        {
            return new StmtTransformResult(stmt.Expression.Visit(this, env), env, false, false, false);
        };
        ...
        // more to check 
        public Func<BinOpDefaultExpr, ICodeTransformEnvironment, IExprTransformResult> VisitBinOpDefaultExpr => (expr, env) =>
        {
            IExprTransformResult left = expr.Left.Visit(this, env);
            IExprTransformResult right = expr.Right.Visit(this, env);
            bool leftintopt = left.Type.IsSubtypeOf(StdLib.Int.Instance.ClassType, true);
        ...
        public Func<DefaultIdentExpr, ICodeTransformEnvironment, IExprTransformResult> VisitDefaultIdentExpr => (expr, env) =>
        {
        ...
        public Func<InstanceExpr, ICodeTransformEnvironment, IExprTransformResult> VisitInstanceExpr => (expr, env) =>
        {
            List<IExprTransformResult> argResults = new List<IExprTransformResult>();
            foreach (IExpr argExp in expr.Arguments)
            {
                try
                {
                    argResults.Add(argExp.Visit(this, env));
            ...
            IParamRef<IClassSpec, Language.IType> classref = env.Context.GetChild(expr.Class.Transform(i => i.Transform(n => n.Name, t => t.TransformType(env.Context)))).Visit(new ParamRefVisitor<object, IParamRef<IClassSpec, Language.IType>, Language.IType>(
                (ns, arg) => throw new TypeCheckException("$0 is not a class", expr.Class),
                (ifc, arg) => throw new TypeCheckException("$0 is not a class", expr.Class),
                (cls, arg) => cls));

            IOptional<ICallableReference> ret = env.GetInstance(classref, argResults.Select(a => a.Type));
        ...
        /**
         * implement the Instance assignment
         */
        public Func<InstanceFieldAssignment, ICodeTransformEnvironment, IStmtTransformResult> VisitInstanceFieldAssignment => (stmt, env) =>
        {

            // check the default expression
            IExprTransformResult exprtr = stmt.Expr.Visit(this, env);
            
            IVariableReference varref;

            //check the identifier is it in the environment 
            IOptional<IVariableReference> localvar = env[stmt.FieldName];

            if (!localvar.HasElem)
            {
                throw new TypeCheckException("No local accessible variable named $0", stmt.FieldName);
            }
            varref = localvar.Elem;
            ...
            AccessorTransformEnvironment<IVariableReference> ate = new AccessorTransformEnvironment<IVariableReference>(env,
             etr =>
             {
                 IOptional<IVariableReference> ret = env.GetFieldAccessVariable(etr, stmt.FieldName);
                 if (!ret.HasElem)
                 {
                     throw new TypeCheckException("Field $0 does not exist", stmt.FieldName);
                 }
                 return ret.Elem;
             },
             qn =>
             {
                 IOptional<IVariableReference> ret = env.Context.GetChild(qn).Visit(new ParamRefVisitor<object, IOptional<IVariableReference>, Language.IType>(
                     (elem, arg) => { throw new TypeCheckException("$0 is not a class", qn); },
                     (elem, arg) => { throw new TypeCheckException("$0 is not a class", qn); },
                     (elem, arg) => { return env.GetStaticFieldAccessVariable(elem, stmt.FieldName); }));
                 if (!ret.HasElem)
                 {
                     throw new TypeCheckException("$0 does not contain a static field named $1", qn, stmt.FieldName);
                 }
                 return ret.Elem;
             },
             etr =>
             {
                 IOptional<IVariableReference> ret = env[stmt.FieldName];
                 if (!ret.HasElem)
                 {
                     throw new TypeCheckException("Field or local variable $0 does not exist", stmt.FieldName);
                 }
                 return ret.Elem;
             });
        ...
        private class AccessorVisitor<T> : Parser.IExprVisitor<AccessorTransformEnvironment<T>, T>
        {
            ...
            public Func<DefaultIdentExpr, AccessorTransformEnvironment<T>, T> VisitDefaultIdentExpr => VisitDefaultIdentExpr;
            ...
            public Func<BinOpDefaultExpr, AccessorTransformEnvironment<T>, T> VisitBinOpDefaultExpr => throw new NotImplementedException();
        }
        ...
        private class CallReceiverVisitor : Parser.IExprVisitor<CallReceiverTransformEnvironment, ICallReceiverTransformResult>
        {
            ...
            public Func<DefaultIdentExpr, CallReceiverTransformEnvironment, ICallReceiverTransformResult> VisitDefaultIdentExpr => VisitDefaultIdentExpr;
            ...
            public Func<BinOpDefaultExpr, CallReceiverTransformEnvironment, ICallReceiverTransformResult> VisitBinOpDefaultExpr => throw new NotImplementedException();
        }
    }
