...
namespace Nom.TypeChecker
{
    internal class TDClass : ATDNamed<TDClassDef>, ITDClass
    {
        ...
        //add instancesdef into the class
        private List<InstanceDef> instances = new List<InstanceDef>();
        public IEnumerable<IInstanceSpec> Instances => instances.ToList();
        IEnumerable<IInstanceDef> IClass.Instances => instances.ToList();
        ...
        public override void ConsolidateDefinition(TDClassDef def)
        {
            ...
            foreach (InstanceDef id in def.Instances)
            {
                if (Instances.Any(xid =>  !xid.Parameters.IsDisjoint(id.Parameters)))
                {
                    throw new TypeCheckException("Instance $0 has non-disjoint overloadings", id.Identifier);
                }
                instances.Add(id);
            }
