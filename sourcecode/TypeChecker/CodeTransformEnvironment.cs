...
namespace Nom.TypeChecker
{
    ...
    internal class UniqueInstanceCall : ICallableReference
    {
        public UniqueInstanceCall(IParameterizedSpecRef<IInstanceSpec> instance)
        {
            Instance = instance;
        }
        public IParameterizedSpecRef<IInstanceSpec> Instance { get; }
        public ICallReceiverTransformResult GenerateCall(IEnumerable<IExprTransformResult> arguments, ICodeTransformEnvironment env)
        {
            List<IInstruction> instructions = new List<IInstruction>();
            if (arguments.Count() != Instance.Element.Parameters.Entries.Count())
            {
                throw new InternalException("Generating call with wrong number of arguments");
            }
            List<IRegister> argregs = new List<IRegister>();
            foreach (var pairing in arguments.Zip(Instance.Element.Parameters.Entries, (arg, entry) => (arg, entry).ToTuple()))
            {
                Language.IType targetType = ((ISubstitutable<Language.IType>)pairing.Item2.Type).Substitute(Instance.Substitutions);
                IExprTransformResult actualArgument = pairing.Item1.EnsureType(targetType, env);
                instructions.AddRange(actualArgument);
                argregs.Add(actualArgument.Register);
            }
            var instr = new CallInstanceCheckedInstruction(Instance, argregs, env.CreateRegister());
            instructions.Add(instr);
            return new CallReceiverTransformResult(new Language.ClassType(Instance.Element.Container, Instance.Substitutions), instr.Register, instructions);
        }
    }
    ...
    public interface ICodeTransformEnvironment
    {
        ...
        IOptional<ICallableReference> GetInstance(IParamRef<IClassSpec, Language.IType> cls, IEnumerable<Language.IType> argTypes);
    ...
    public abstract class ACodeTransformEnvironment : ICodeTransformEnvironment
    {
        ...
        public abstract IOptional<ICallableReference> GetInstance(IParamRef<IClassSpec, Language.IType> ils, IEnumerable<Language.IType> argTypes);
    ...
    internal abstract class ATopLevelCodeTransformEnvironment : ACodeTransformEnvironment
    {
        ...
        public override IOptional<ICallableReference> GetInstance(IParamRef<IClassSpec, Language.IType> ils, IEnumerable<Language.IType> argTypes)
        {

            List<IInstanceSpec> instances = ils.Element.Instances.Where(c => c.Visibility >= this.Context.MembersVisibleAt(ils.Element) && c.Parameters.WouldAccept(ils.PArguments, argTypes)).ToList();
            ...
            {
               return new UniqueInstanceCall(new ParameterizedSpecRef<IInstanceSpec>(instances.Single(), ils.PArguments.Transform<Language.ITypeArgument>(t => t))).InjectOptional();
                
            }
            return new OverloadedInstanceCall(instances,ils.PArguments).InjectOptional();
        }
        ...
        protected class OverloadedInstanceCall : ICallableReference
        {
            public OverloadedInstanceCall(IEnumerable<IInstanceSpec> instances, ITypeEnvironment<Language.IType> env)
            {
                Instances = instances;
                Environment = env;
            }
            public IEnumerable<IInstanceSpec> Instances { get; }
            public ITypeEnvironment<Language.IType> Environment { get; }
            public ICallReceiverTransformResult GenerateCall(IEnumerable<IExprTransformResult> arguments, ICodeTransformEnvironment env)
            {
                throw new NotImplementedException();
    ...
    internal abstract class AUnscopedChildCodeTransformEnvironment : ICodeTransformEnvironment
    {
        ...
        public IOptional<ICallableReference> GetInstance(IParamRef<IClassSpec, Language.IType> ilc, IEnumerable<Language.IType> argTypes)
        {
            return ParentEnv.GetInstance(ilc, argTypes);
        }
    ...
    public abstract class AScopedChildTransformEnvironment : ACodeTransformEnvironment
    {
        ...
        public override IOptional<ICallableReference> GetInstance(IParamRef<IClassSpec, Language.IType> ilc, IEnumerable<Language.IType> argTypes)
        {
            return Parent.GetInstance(ilc, argTypes);
        }
    ...
    internal class InstanceTransformEnvironment : AInstanceMethodCodeTransformEnvironment
    {
        ...
        public InstanceTransformEnvironment(TDClass cls, ITypeParametersSpec typeParameters, IParametersSpec parameters, Language.IType expectedReturnType, ITypeCheckLookup<Language.IType, Language.ITypeArgument> context) : base(cls, typeParameters, parameters, context, expectedReturnType.InjectOptional())
        {
            EnableThis();
            foreach (var field in Class.LocalFields)
            {
