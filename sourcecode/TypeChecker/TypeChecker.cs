...
namespace Nom.TypeChecker
{
    public class TypeChecker
    {
        ...
        private class TypeCheckBlock
        {
            ...
            // add the dictionary for the Instance 
            public readonly Dictionary<Parser.InstanceDef, InstanceDef> Instance = new Dictionary<Parser.InstanceDef, InstanceDef>();
            ...
            //constructor of the typecheckerblock
            ...
            private class InitializeMembersPass : TypeCheckPass
            {
                ...
                protected override void HandleClass(ClassDef elem, ClassTypeCheckInfo tci, TypeCheckBlock tcb, ITypeCheckLookup<Language.IType, Language.ITypeArgument> context)
                {
                    ...
                    foreach (Parser.Constructor cons in elem.Constructors)
                    {
                        ...
                        List<TDTypeArgDeclDef> typeArgs = new List<TDTypeArgDeclDef>();
                        int startIndex = tci.Container.ParentNamespace.Extract(ns => ns.OverallTypeParameterCount, 0);
                  
                        List<TypeArgDecl> list = new List<TypeArgDecl>();
                      
                            foreach (VarDecl tad in cons.Args)
                            {
                                list.Add(new TypeArgDecl(tad.Name,tad.Type,tad.Type));
                            }
                    ...
                    //add the instance to the tcb and instance infomation in tci
                    foreach (Parser.InstanceDef ins in elem.Instances)
                    {
                        TypeParametersSpec tps = GenerateParametersSpec(ins.TypeArguments, tci.Container.OverallTypeParameterCount, context);
                        var ihcontext = context.PushVariables(tps);
                        ParametersSpec ps = new ParametersSpec(ins.Arguments.Select(arg => new ArgumentDeclDef(arg.Name, arg.Type.TransformType(ihcontext))));
                        InstanceDef cd = new InstanceDef(ins.Name, tps, ps, new Language.ClassType(tci.Container, context.ContextRoot.PArguments.Transform<Language.ITypeArgument>(t => t)), ins.Visibility.Visibility, tci.Container);
                        tcb.Instance[ins] = cd;
                        tci.Definition.AddInstanceDef(cd);
                    }
                }
            }
                ...
                protected override void HandleClass(ClassDef elem, ClassTypeCheckInfo tci, TypeCheckBlock tcb, ITypeCheckLookup<LType, LTypeArg> context)
                {
                    foreach (Parser.Constructor cdef in elem.Constructors)
                    {
                        try
                        {
                            ...
                            //check the class field is it initialize
                            ...
                            //before this everything is not initialized
                            //this step is for initialized
                    ...
                    /*
                     *writing the main type checking in instance class
                     */
                    foreach (Parser.InstanceDef idef in elem.Instances)
                    {
                        try
                        {
                            InstanceDef id = tcb.Instance[idef];
                            InstanceTransformEnvironment ite = new InstanceTransformEnvironment(tci.Container, id.TypeParameters, id.Parameters, id.ReturnType, context);
                            //check the assignment field
                            
                            IStmtTransformResult str = idef.code.Visit(CodeTransformer.Instance, ite);
                            List<IStmtTransformResult> defaultInitializers = new List<IStmtTransformResult>();
                            List<IInstruction> fieldAssignments = new List<IInstruction>();
                            //check the field of the assignment 
                            foreach (FieldSpec fs in tci.Container.Fields)
                            {
                                bool pass = false;
                                IVariableReference localVar = ite.GetFieldVariable(fs);
                        
                                foreach (var a in idef.Assignments)
                                {
                                    if (fs.Identifier.ToString() == a.FieldName.ToString())
                                    {
                                        pass = true;
                                    }
                                }
                            ...
                            int index = 0;
                            foreach (PhiNode pn in id.Instructions.Where(x => x is PhiNode))
                            {
                                pn.Index = index++;
                            }
        ...
        public Nom.TypeChecker.Program TypeCheck(Nom.Parser.Program program, IEnumerable<ILibrary> libraries, string programName)
        {
            ...
            {
                //create block base on the code file generated by Parser
