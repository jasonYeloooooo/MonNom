...
namespace Nom
{
	namespace Runtime
	{
		...
		NomInstruction* BytecodeReader::ReadInstruction() {
			...
			case OpCode::CallInstance:{
				RegIndex reg = stream.read<RegIndex>();
				LocalConstantID cls = stream.read <LocalConstantID>();
				LocalConstantID typeArgs = stream.read <LocalConstantID>();
				return new CallInstance(reg, context.GetGlobalID(cls), context.GetGlobalID(typeArgs));
			}
		...
		NomClass* BytecodeReader::ReadClass(NomModule* mod, BytecodeTopReadHandler* handler) {
			...
			uint64_t instanceCount = stream.read<uint64_t>();
		...
		// to do :
		NomInstance* BytecodeReader::ReadInstance(NomClassLoaded* cls) {
			if (stream.read_char() != (unsigned char)BytecodeInternalElementType::Instance) {
				throw "Expected Instance, but did not encounter Instance marker";
			}
			ConstantID argTypes = context.GetGlobalID(stream.read<LocalConstantID>());
			RegIndex regcount = stream.read<RegIndex>();
			NomInstanceLoaded* ins = cls->AddInstance(NULL,argTypes, regcount);
			uint64_t instructionCount = stream.read<uint64_t>();
			while (instructionCount > 0) {
				auto instr = ReadInstruction();
				if (NomVerbose)
