...
#pragma region InstanceCompileEnv
		InstanceCompileEnv::InstanceCompileEnv(RegIndex regcount, const llvm::Twine contextName, llvm::Function* function, const std::vector<PhiNode*>* phiNodes, const llvm::ArrayRef<NomTypeParameterRef> directTypeArgs, const TypeList argtypes, NomClassTypeRef thisType, const NomInstance* method) : AFullArityCompileEnv(regcount, contextName, function, phiNodes, directTypeArgs, method, argtypes, thisType), Method(method)
		{
		}

		NomTypeVarValue InstanceCompileEnv::GetTypeArgument(NomBuilder& builder, int i)
		{
			return TypeArguments[i];
		}
		size_t InstanceCompileEnv::GetEnvTypeArgumentCount()
		{
			return 0;
		}
		llvm::Value* InstanceCompileEnv::GetEnvTypeArgumentArray(NomBuilder& builder)
		{
			return ConstantPointerNull::get(TYPETYPE->getPointerTo());
		}
		bool InstanceCompileEnv::GetInInstance() { return inInstance; }
		void InstanceCompileEnv::SetPastInitialSetup() { inInstance = false; }
