...
namespace Nom
{
	namespace Runtime {
		...
		public:
			...
			std::vector<NomInstance*> Instances;
			...
			NomValue GenerateInstanceCall(NomBuilder& builder, CompileEnv* env, const TypeList typeArgs, llvm::Value* objpointer, llvm::ArrayRef<NomValue> args) const;
			...
			NomInstantiationRef<const NomInstance> GetInstance(const TypeList typeArgs, const TypeList argTypes) const;
		...
		public:
			...
			void AddInstance(NomInstance* instance)
			{
		...
		public:
			...
			NomInstanceLoaded* AddInstance(const ConstantID name, const ConstantID arguments, const RegIndex regcount);
			...
			virtual void PushDependencies(std::set<ConstantID>& set) const override
			{
				...
				//add instance to the dependenci
				for (auto instr : Instances)
				{
					dynamic_cast<NomCallableLoaded*>(instr)->PushDependencies(set);
				}
