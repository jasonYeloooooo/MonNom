...
namespace Nom
{
	namespace Runtime
	{
		...
		NomInstanceLoaded* NomClassLoaded::AddInstance(const ConstantID name, const ConstantID arguments, const RegIndex regcount) {
			const std::string sname = "_Instance_" + name;
			//std::string sname = "_Instance_" + this->GetName()->ToStdString() + "_" + std::to_string(Constructors.size());

			NomInstanceLoaded* ins = new NomInstanceLoaded(this, sname, sname, arguments, regcount, 0, false);
			NomClass::AddInstance(ins);
			return ins;
		}
		...
		void NomClass::CompileLLVM(Module* mod) const
		{
			...
			for (auto ins = Instances.begin(); ins != Instances.end(); ins++)
			{
				(*ins)->GetLLVMElement(*mod);
			}
		...
		NomValue NomClass::GenerateInstanceCall(NomBuilder& builder, CompileEnv* env, const TypeList typeArgs, llvm::Value* objpointer, llvm::ArrayRef<NomValue> args) const
		{
			auto argssize = args.size();
			NomTypeRef* argTypesBuf = (NomTypeRef*)(nmalloc(sizeof(NomTypeRef) * argssize));
			llvm::Value** argValuesBuf = (llvm::Value**)(nmalloc(sizeof(llvm::Value*) * (argssize + 1 + typeArgs.size())));
			...
			{
				if (instance->Satisfies(&NomSubstitutionContextList::EmptyContext(), typeArgs, argTypes)) //TODO: make pessimistic and go to dispatcher later
				{
					auto fun = instance->GetLLVMFunction(env->Module);
					auto funtype = fun->getFunctionType();
					auto instanceSignature = instance->Signature(&nscl);
					for (size_t i = 0; i < argssize; i++)
					{
						if (!argTypesBuf[i]->IsSubtype(instanceSignature.ArgumentTypes[i], false))
						{
							argValuesBuf[argspos + i] = CastInstruction::MakeCast(builder, env, args[i], instanceSignature.ArgumentTypes[i]);
						}
						auto paramtype = funtype->getParamType(argspos + i);
						if (paramtype->isIntegerTy() && !argValuesBuf[argspos + i]->getType()->isIntegerTy())
						{
		...
		NomInstantiationRef<const NomInstance> NomClass::GetInstance(const TypeList typeArgs, const TypeList argTypes) const
		{
			for (auto instance: Instances)
			{
				if (instance -> Satisfies(&NomSubstitutionContextList::EmptyContext(), typeArgs, argTypes))
				{
					return NomInstantiationRef<const NomInstance>(instance, typeArgs);
				}
			}
			throw new std::exception();
		}
