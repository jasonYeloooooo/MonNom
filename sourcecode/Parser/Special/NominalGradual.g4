...
defaultexpr returns [IDefaultExpr exp]
	: STRING {$exp = new DefaultStringExpr($STRING);}
	 ...
	 | ctype COLONCOLON ident LPAREN (elems+=defaultexpr (COMMA elems+=defaultexpr)*)? RPAREN {$exp = new DefaultInstanceExpr($ctype.ct, $ident.i, $elems.Select(e=>e.exp), $ctype.ct.Start.SpanTo($RPAREN));}
	 |<assoc=left> e1=defaultexpr addsubop e2=defaultexpr {$exp = new BinOpDefaultExpr($e1.exp,$addsubop.op,$e2.exp,$e1.exp.Start.SpanTo($e2.exp.End));}
	 |<assoc=left> e1=defaultexpr divmultop e2=defaultexpr {$exp = new BinOpDefaultExpr($e1.exp,$divmultop.op,$e2.exp,$e1.exp.Start.SpanTo($e2.exp.End));}
	 ;
...
instancedecl returns [InstanceDef i]
	...
	| (v+=visibility)? m=MULTI INSTANCE id =ident LPAREN (iargs+=argdecl (COMMA iargs+=argdecl)*)? RPAREN (EXTENDS sid+=ident (LPAREN (sargs+=defaultexpr (COMMA sargs+=defaultexpr)*)? RPAREN)?)? LBRACE ifas+=instanceassignment* RBRACE {$i=new InstanceDef($v.SingleOrDefault()?.v??VisibilityNode.Private, $id.i, $sid.Select(s=>s.i).SingleOrDefault(), $ifas.Select(fa=>fa.ifa), $m!=null, false, $sargs.Select(s=>s.exp), $iargs.Select(a=>a.v.ToArgDecl()), $INSTANCE.ToSourceSpan($RBRACE));}
	;

instanceassignment returns [InstanceFieldAssignment ifa]
	: ident EQ defaultexpr SEMICOLON {$ifa = new InstanceFieldAssignment($ident.i, $defaultexpr.exp, $EQ.ToSourceSpan($SEMICOLON));}
